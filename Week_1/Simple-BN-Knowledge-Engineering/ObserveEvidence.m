...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % find out which variable to be reduced in F(j).var
            [~, observedVar] = intersect(F(j).var, v);
            % get assignment of all variables
            assignment = IndexToAssignment(1: prod(F(j).card), F(j).card);
            
            % find the assignment index that equals to the observed evidence
            [assignmentIndex, ~] = find(assignment(:, observedVar) ~= x);
            % get corresponding assignment
            assignment = assignment(assignmentIndex, :);
            
            % set the assignments to 0
            for a = 1: length(assignment)
                F(j) = SetValueOfAssignment(F(j), assignment(a, :), [0]);
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
