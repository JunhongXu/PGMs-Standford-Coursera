...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
for i=1: numPeople
    % get parent of the current person
    parent = pedigree.parents(i,:);
    % if the person does not have parents info, then P(G|Freq)
    if ismember(parent, [0, 0], 'rows')
        genotypeFactor = genotypeGivenAlleleFreqsFactor(alleleFreqs, i);
    else
        parentOne = parent(1);
        parentTwo = parent(2);
        genotypeFactor = genotypeGivenParentsGenotypesFactor(numAlleles, i, parentOne, parentTwo);
    end
    % calculate phenotypeFactor
    phenotypeFactor = phenotypeGivenGenotypeFactor(alphaList, i, i+numPeople);
    factorList(i) = genotypeFactor;
    factorList(i+numPeople) = phenotypeFactor;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  