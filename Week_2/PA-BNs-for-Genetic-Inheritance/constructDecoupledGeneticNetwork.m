...
for i = 1: numPeople
   ...
   if all([0, 0] == pedigree.parents(i, :))
       personGeneCopyOne = childCopyGivenFreqsFactor(alleleFreqs, i);
       personGeneCopyTwo = childCopyGivenFreqsFactor(alleleFreqs, i + numPeople);
   else
       parent = pedigree.parents(i, :);
       personGeneCopyOne = childCopyGivenParentalsFactor(numAlleles, i, parent(1), parent(1) + numPeople);
       personGeneCopyTwo = childCopyGivenParentalsFactor(numAlleles, i+numPeople, parent(2), parent(2) + numPeople);
   end
   phenotype = phenotypeGivenCopiesFactor(alphaList, numAlleles, i, i + numPeople, i + 2 * numPeople);
   factorList(i) = personGeneCopyOne;
   factorList(i + numPeople) = personGeneCopyTwo;
   factorList(i + 2 * numPeople) = phenotype;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  