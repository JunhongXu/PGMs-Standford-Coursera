...
phenotypeFactor.var = [phenotypeVar, reshape(geneCopyVarOneList, 1, []), reshape(geneCopyVarTwoList, 1, [])];
% Fill in phenotypeFactor.card.  This should be a 1-D row vector.
[numVarGeneOne, ~] = size(geneCopyVarOneList);
[numVarGeneTwo, ~] = size(geneCopyVarTwoList);
card = ones(1, numVarGeneOne + numVarGeneTwo + 1) * 2;
phenotypeFactor.card = card;
phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
assignment = IndexToAssignment(1:prod(card), card);
% [1, 1; 1, 1; 2, 1; ...]
alleles = assignment(:, 2:end);
[~, col] = size(alleles);
for i=1: length(alleles)
    ...
    for j=1: length(allele)
        ...
        f = f + alleleWeights{geneIndex}(allele(j));
    end
    % compute sigmoid factor
    sigmoid = computeSigmoid(f);
    phenotypeFactor.val(i) = sigmoid;
end
phenotypeFactor.val(2:2:end) = 1 - phenotypeFactor.val(2:2:end);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%