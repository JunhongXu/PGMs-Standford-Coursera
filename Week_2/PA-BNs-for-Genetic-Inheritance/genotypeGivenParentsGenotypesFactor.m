...
parentOneAlleles = genotypesToAlleles(assignment(:, 2), :);
parentTwoAlleles = genotypesToAlleles(assignment(:, 3), :);
childAlleles = genotypesToAlleles(assignment(:, 1), :);

for i=1: length(parentOneAlleles)
    allelesRow = nchoosek([parentOneAlleles(i, :), parentTwoAlleles(i, :)], 2);
    allelesRow = allelesRow(2: length(allelesRow)-1, :);
    memberIndexOne = ismember(allelesRow, childAlleles(i, :), 'rows');
    memberIndexTwo = ismember(allelesRow, fliplr(childAlleles(i, :)), 'rows');
    memberIndex = memberIndexOne | memberIndexTwo;
    genotypeFactor.val(i) = sum(memberIndex) / length(memberIndex);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%