...
factorAssignment = IndexToAssignment(1:prod(phenotypeFactor.card), phenotypeFactor.card);
allelesIndex = factorAssignment(:, 2:end);
linearIndex = sub2ind(size(allelesToGenotypes), allelesIndex(:, 1), allelesIndex(:, 2));
genotypeIDs = allelesToGenotypes(linearIndex);
phenotypeProbs = alphaList(genotypeIDs);
phenotypeProbs = reshape(phenotypeProbs, [], numel(phenotypeProbs));
phenotypeProbs(2:2:end) = 1 - phenotypeProbs(1:2:end);
phenotypeFactor.val = phenotypeProbs;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%